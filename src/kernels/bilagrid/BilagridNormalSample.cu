#include "kernels/bilagrid/BilagridNormalSampleFwd_kernel.cuh"
#include "kernels/bilagrid/BilagridNormalSampleBwdV1_kernel.cuh"
#include "kernels/bilagrid/BilagridNormalSampleBwdV2_kernel.cuh"  // non-PATCHED scatter
...
void bilagrid_normal_uniform_sample_forward(
    ...
    bilagrid_normal_uniform_sample_forward_kernel<<<blocks, threads, 0, stream>>>(
        bilagrid, rgb, output,
        N, L, H, W, h, w,
        grid_indices
    );
...
void bilagrid_normal_patched_sample_forward(
    ...
    int threads = 256;
    int blocks = (total + threads - 1) / threads;
    bilagrid_normal_patched_sample_forward_kernel<<<blocks, threads, 0, stream>>>(
        bilagrid, rgb, output,
...
void bilagrid_normal_uniform_sample_backward_v1(
    ...
    if (v_rgb != nullptr) {
        ...
        bilagrid_normal_uniform_sample_backward_v1_kernel_normal<<<blocks, kBilagridBwdV1RgbThreads, 0, stream>>>(
...
void bilagrid_normal_patched_sample_backward_v1(
    ...
    // v_bilagrid
    {
        ...
        bilagrid_normal_patched_sample_backward_v1_kernel_bilagrid<<<bounds, block, 0, stream>>>(
            bilagrid, rgb, v_output, v_bilagrid,
    ...
    // v_rgb
    {
        int total = N * m * h * w;
        int blocks = (total + kBilagridBwdV1RgbThreads - 1) / kBilagridBwdV1RgbThreads;
        bilagrid_normal_patched_sample_backward_v1_kernel_normal<<<blocks, kBilagridBwdV1RgbThreads, 0, stream>>>(
...
void bilagrid_normal_uniform_sample_backward_v2(
    ...
    int blocks = (total + threads - 1) / threads;
    bilagrid_normal_uniform_sample_backward_v2_kernel<<<blocks, threads, 0, stream>>>(
        bilagrid, rgb, v_output, v_bilagrid,

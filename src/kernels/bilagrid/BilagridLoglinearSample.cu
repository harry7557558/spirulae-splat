#include "kernels/bilagrid/BilagridLoglinearSampleFwd_kernel.cuh"
#include "kernels/bilagrid/BilagridLoglinearSampleBwdV1_kernel.cuh"
#include "kernels/bilagrid/BilagridLoglinearSampleBwdV2_kernel.cuh"  // non-PATCHED scatter
...
void bilagrid_loglinear_uniform_sample_forward(
    ...
    bilagrid_loglinear_uniform_sample_forward_kernel<<<blocks, threads, 0, stream>>>(
        bilagrid, rgb, output,
        N, L, H, W, h, w,
        grid_indices
    );
...
void bilagrid_loglinear_patched_sample_forward(
    ...
    int threads = 256;
    int blocks = (total + threads - 1) / threads;
    bilagrid_loglinear_patched_sample_forward_kernel<<<blocks, threads, 0, stream>>>(
        bilagrid, rgb, output,
...
void bilagrid_loglinear_patched_sample_backward_v1(
    ...
    // v_bilagrid
    {
        ...
        bilagrid_loglinear_patched_sample_backward_v1_kernel_bilagrid<<<bounds, block, 0, stream>>>(
            bilagrid, rgb, v_output, v_bilagrid,
...
void bilagrid_loglinear_uniform_sample_backward_v2(
    ...
    int blocks = (total + threads - 1) / threads;
    bilagrid_loglinear_uniform_sample_backward_v2_kernel<<<blocks, threads, 0, stream>>>(

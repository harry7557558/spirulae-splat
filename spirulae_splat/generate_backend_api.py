"""Generate backend API module headers (csrc/backend/api/) as forwarders.
...
Run from repo root: python3 spirulae_splat/generate_backend_api.py
"""

import os

CSRC = "spirulae_splat/splat/cuda/csrc/"
OUT_DIR = CSRC + "backend/api/"

# module header -> (per-kernel headers, one-line description)
MODULES = {
    "Projection.h": (
        ["ProjectionFwd.cuh", "ProjectionBwd.cuh", "ProjectionPackedFwd.cuh",
         "ProjectionBwdQuantGrad.cuh", "FusedProjectionBwdOptim.cuh"],
    ...
    "Rasterization.h": (
        ["RasterizationFwd.cuh", "RasterizationBwd.cuh",
         "RasterizationEval3DFwd.cuh", "RasterizationEval3DBwd.cuh",
         "RasterizationMomentsFwd.cuh"],
        "Tile-based rasterization forward/backward (2D and eval-3D/3DGUT).",
    ),
    "TileIntersect.h": (
        ["IntersectTile.cuh", "SplatTileIntersector.cuh"],
    ...
    "Loss.h": (
        ["FusedSSIM.cuh", "PerPixelLoss.cuh", "PerSplatLoss.cuh"],
    ...
    "Bilagrid.h": (
        ["BilagridUtils.cuh", "BilagridBindings.h"],

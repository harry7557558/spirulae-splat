...
function's declaration in the per-kernel csrc/*.cuh headers, and emits one
...
CSRC = REPO / "spirulae_splat" / "splat" / "cuda" / "csrc"
...
def main():
    ...
    headers = {}
    for hdr in sorted(CSRC.glob("*.cuh")) + sorted(CSRC.glob("*.h")):
    ...
    lines = []
    lines.append("// GENERATED by spirulae_splat/generate_vulkan_stubs.py — "
                 "DO NOT EDIT.")
    lines.append("//")

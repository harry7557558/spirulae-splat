...
name = "spirulae_splat_cuda"
build_dir = _get_build_directory(name, verbose=False)
extra_include_paths = [os.path.join(PATH, "csrc/glm")]
extra_cflags = ["-O3"]
...
try:
    ...
    if cuda_toolkit_available():
        ...
        if os.path.exists(os.path.join(build_dir, "spirulae_splat_cuda.so")) or os.path.exists(
            os.path.join(build_dir, "spirulae_splat_cuda.lib")
        ):
            ...
            with Console().status(
                "[bold yellow]spirulae_splat: Setting up CUDA (This may take a few minutes the first time)",
        ...
        Console().print(
            "[yellow]spirulae_splat: No CUDA toolkit found. spirulae_splat will be disabled.[/yellow]"
        )

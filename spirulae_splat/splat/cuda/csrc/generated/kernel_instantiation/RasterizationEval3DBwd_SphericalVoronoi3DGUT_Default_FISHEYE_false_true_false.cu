// This file is auto generated by `generate_kernel_instantiation.py`

#define NO_TORCH
#include "Primitive3DGUT_SV.cuh"
#include "RasterizationEval3DBwd_kernel.cuh"

template void rasterize_to_pixels_eval3d_bwd_kernel_wrapper<
    SphericalVoronoi3DGUT_Default,
    gsplat::CameraModelType::FISHEYE,
    false,
    true,
    false
>(
    cudaStream_t stream,
    const uint32_t I,
    const uint32_t n_isects,
    // fwd inputs
    SphericalVoronoi3DGUT_Default::Screen::Buffer splat_buffer,
    const float *__restrict__ viewmats, // [B, C, 4, 4]
    const float4 *__restrict__ intrins,  // [B, C, 4], fx, fy, cx, cy
    const CameraDistortionCoeffsBuffer dist_coeffs_buffer,
    const float *__restrict__ backgrounds, // [..., CDIM] or [nnz, CDIM]
    const bool *__restrict__ masks,           // [..., tile_height, tile_width]
    const uint32_t image_width,
    const uint32_t image_height,
    const uint32_t tile_width,
    const uint32_t tile_height,
    const int32_t *__restrict__ tile_offsets, // [..., tile_height, tile_width]
    const int32_t *__restrict__ flatten_ids,  // [n_isects]
    // fwd outputs
    const float *__restrict__ render_Ts,      // [..., image_height, image_width, 1]
    const int32_t *__restrict__ last_ids, // [..., image_height, image_width]
    SphericalVoronoi3DGUT_Default::RenderOutput::Buffer render_output_buffer,
    SphericalVoronoi3DGUT_Default::RenderOutput::Buffer render2_output_buffer,
    const float *__restrict__ loss_map_buffer,           // [..., image_height, image_width, 1]
    // grad outputs
    SphericalVoronoi3DGUT_Default::RenderOutput::Buffer v_render_output_buffer,
    const float *__restrict__ v_render_alphas, // [..., image_height, image_width, 1]
    SphericalVoronoi3DGUT_Default::RenderOutput::Buffer v_distortions_output_buffer,
    // grad inputs
    SphericalVoronoi3DGUT_Default::Screen::Buffer v_splat_buffer,
    SphericalVoronoi3DGUT_Default::Screen::Buffer vr_splat_buffer,
    SphericalVoronoi3DGUT_Default::Screen::Buffer h_splat_buffer,
    float *__restrict__ v_viewmats // [B, C, 4, 4]
);

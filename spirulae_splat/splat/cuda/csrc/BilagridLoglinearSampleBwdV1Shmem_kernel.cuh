// Draft: shared-memory corner preload variant of the bilagrid LOGLINEAR
// sample backward v1 grid-grad kernel.
//
// The log-linear grid stores a 9-channel affine matrix per cell, but the
// backward pass only reads the 3 DIAGONAL channels (full-channel indices
// {0, 4, 8}) to compute the exp-scaled per-channel grad. We therefore
// preload only those 3 channels per cell, giving a 10 x 10 x 3 x 3 x 4 =
// 3.6 KB footprint per block (one third of the PPISP shmem cost).
//
// Layout: shmem_corners[lz][ly][lx][diag_idx] where diag_idx in {0, 1, 2}
// maps to full-channel indices {0, 4, 8} via 4 * diag_idx (= 3 * diag_idx
// + diag_idx).
...
#ifdef PATCHED
__global__ void bilagrid_loglinear_patched_sample_backward_v1_kernel_bilagrid_shmem(
#else
__global__ void bilagrid_loglinear_uniform_sample_backward_v1_kernel_bilagrid_shmem(
#endif

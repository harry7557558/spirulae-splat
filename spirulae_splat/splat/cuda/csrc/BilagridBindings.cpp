...
// Grad-equality test entrypoint: runs only the grid-grad pass, with a
// runtime choice between the original and shmem-preload kernel.
//
// Tensor shapes (channel-last, matches the engine's `grids.resize(N, L, H, W, C)`
// layout that the kernels index against):
//   bilagrid  [N, L, H, W, C]   (C = 9 ppisp / 9 loglinear / 3 normal / 2 depth)
//   rgb/depth [N, h, w, c_img]  (c_img = 3 / 1)
//   v_output  matches rgb/depth shape
//   v_bilagrid matches bilagrid shape (output)
//
// NOTE: the older `bilagrid_*_uniform_sample_backward_tensor` wrappers read
// dims (2, 3, 4) for (L, H, W) -- that pattern assumes a channel-SECOND
// shape (N, C, L, H, W) which does not match the kernel's storage layout.
// These grad-grad wrappers read (1, 2, 3) instead, which is the correct
// channel-last extraction.
void bilagrid_ppisp_uniform_sample_backward_grid_grad_tensor(
    TorchTensorView bilagrid, TorchTensorView rgb, TorchTensorView v_output,
    TorchTensorView v_bilagrid,
    int target_tile_size, bool use_shmem
) {
    int N = (int)_tv_dim(bilagrid, 0), L = (int)_tv_dim(bilagrid, 1),
        H = (int)_tv_dim(bilagrid, 2), W = (int)_tv_dim(bilagrid, 3);
    int h = (int)_tv_dim(rgb, 1), w = (int)_tv_dim(rgb, 2);
    bilagrid_ppisp_uniform_sample_backward_v1_choice(
        _tv_cf(bilagrid), _tv_cf(rgb), _tv_cf(v_output),
        _tv_f(v_bilagrid),
        N, L, H, W, h, w,
        target_tile_size, use_shmem, kStream);
}
...
void bilagrid_loglinear_uniform_sample_backward_grid_grad_tensor(
    ...
    int N = (int)_tv_dim(bilagrid, 0), L = (int)_tv_dim(bilagrid, 1),
        H = (int)_tv_dim(bilagrid, 2), W = (int)_tv_dim(bilagrid, 3);
    int h = (int)_tv_dim(rgb, 1), w = (int)_tv_dim(rgb, 2);
    bilagrid_loglinear_uniform_sample_backward_v1_choice(
        _tv_cf(bilagrid), _tv_cf(rgb), _tv_cf(v_output),
        _tv_f(v_bilagrid),
        N, L, H, W, h, w,
        target_tile_size, use_shmem, kStream);
...
void bilagrid_normal_uniform_sample_backward_grid_grad_tensor(
    ...
    int N = (int)_tv_dim(bilagrid, 0), L = (int)_tv_dim(bilagrid, 1),
        H = (int)_tv_dim(bilagrid, 2), W = (int)_tv_dim(bilagrid, 3);
    int h = (int)_tv_dim(rgb, 1), w = (int)_tv_dim(rgb, 2);
    bilagrid_normal_uniform_sample_backward_v1_choice(
        _tv_cf(bilagrid), _tv_cf(rgb), _tv_cf(v_output),
        _tv_f(v_bilagrid),
        N, L, H, W, h, w,
        target_tile_size, use_shmem, kStream);
...
void bilagrid_depth_uniform_sample_backward_grid_grad_tensor(
    ...
    TorchTensorView v_output, TorchTensorView v_bilagrid,
    int target_tile_size, bool use_shmem
) {
    int N = (int)_tv_dim(bilagrid, 0), L = (int)_tv_dim(bilagrid, 1),
        H = (int)_tv_dim(bilagrid, 2), W = (int)_tv_dim(bilagrid, 3);
    int h = (int)_tv_dim(depth, 1), w = (int)_tv_dim(depth, 2);
    bilagrid_depth_uniform_sample_backward_v1_choice(

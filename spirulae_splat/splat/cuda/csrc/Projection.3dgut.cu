...
> projection_3dgut_hetero_forward_tensor(
    ...
    uint32_t N = Vanilla3DGUT<0>::WorldBuffer(in_splats).size();  // number of splats
    ...
    if (nnz != 0) {
        ...
        #define LAUNCH(n) if (sh_degree == n) { \
            if (cmt(camera_model) == ssplat::CameraModelType::PINHOLE) \
                ...
                projection_hetero_forward_kernel<Vanilla3DGUT<n>, ssplat::CameraModelType::FISHEYE> _LAUNCH_ARGS; \
            else if (cmt(camera_model) == ssplat::CameraModelType::EQUISOLID) \
                projection_hetero_forward_kernel<Vanilla3DGUT<n>, ssplat::CameraModelType::EQUISOLID> _LAUNCH_ARGS; \
            else \
...
> projection_3dgut_hetero_backward_tensor(
    ...
    if (nnz != 0) {
        ...
        #define LAUNCH(n) if (sh_degree == n) { \
            if (cmt(camera_model) == ssplat::CameraModelType::PINHOLE) \
                projection_3dgs_hetero_backward_kernel<Vanilla3DGUT<n>, ssplat::CameraModelType::PINHOLE> _LAUNCH_ARGS; \
            else if (cmt(camera_model) == ssplat::CameraModelType::FISHEYE) \
                projection_3dgs_hetero_backward_kernel<Vanilla3DGUT<n>, ssplat::CameraModelType::FISHEYE> _LAUNCH_ARGS; \
            else if (cmt(camera_model) == ssplat::CameraModelType::EQUISOLID) \
                projection_3dgs_hetero_backward_kernel<Vanilla3DGUT<n>, ssplat::CameraModelType::EQUISOLID> _LAUNCH_ARGS; \
            else \
                throw std::runtime_error("Unsupported camera model"); \
        }
        LAUNCH(3)

...
template <typename SplatPrimitive, ssplat::CameraModelType camera_model, bool output_distortion>
__global__ void rasterize_to_pixels_sorted_eval3d_bwd_kernel(
    ...
    RenderOutput v_pix_colors = (inside ?
            v_render_output_buffer.load<SplatPrimitive::pixelType>(pix_id_image_global)
             : RenderOutput::zero());
    // float pix_background[CDIM];  // TODO

    RenderOutput pix_colors;
    RenderOutput pix2_colors;
    RenderOutput v_distortion_out;
    ...
    for (int32_t t = range_end-1; t >= (int)range_start - (int)MAX_PQUEUE_SIZE-1; t--) {
        ...
        if (pqueue_size >= MAX_PQUEUE_SIZE || (t < range_start && pqueue_size != 0)) {
            ...
            if (output_distortion) {
                ...
                // \frac{dL}{ds}=0
                RenderOutput v_dist = v_distortion_out;
                RenderOutput c0 =
            ...
            v_splat.atomicAddToBuffer(v_splat_buffer, splat_idx, gaussian_ids);
        }

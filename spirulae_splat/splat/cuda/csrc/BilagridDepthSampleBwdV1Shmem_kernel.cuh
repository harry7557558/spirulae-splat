// Draft: shared-memory corner preload variant of the bilagrid DEPTH sample
// backward v1 grid-grad kernel.
//
// Same structure as the PPISP / NORMAL shmem variants. The depth grid is
// 2-channel (scale, bias); footprint = 10 x 10 x 3 x 2 x 4 = 2.4 KB per
// block. Depth scaling is per-camera so `scalar` is loaded once per block
// before the preload.
...
#ifdef PATCHED
__global__ void bilagrid_depth_patched_sample_backward_v1_kernel_bilagrid_shmem(
#else
__global__ void bilagrid_depth_uniform_sample_backward_v1_kernel_bilagrid_shmem(
#endif

...
void bilagrid_normal_uniform_sample_backward_v2(
    ...
    bilagrid_normal_uniform_sample_backward_v2_kernel<<<bounds, block, 0, stream>>>(
        bilagrid, rgb, v_output,
        v_bilagrid, v_rgb,
        N, L, H, W, m, h, w
    );
...
void bilagrid_normal_patched_sample_backward_v2(
    BilagridReader bilagrid,
    const float* rgb,
    const int* offsets,
    const float* v_output,
    ...
    bilagrid_normal_patched_sample_backward_v2_kernel<<<bounds, block, 0, stream>>>(
        bilagrid, rgb, v_output,
        v_bilagrid, v_rgb,
        N, L, H, W, m, h, w, h0, w0, offsets
    );

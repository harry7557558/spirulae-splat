// Draft: shared-memory corner preload variant of the bilagrid NORMAL sample
// backward v1 grid-grad kernel.
//
// Structure mirrors BilagridPpispUniformSampleBwdV1Shmem_kernel.cuh: a
// 10 x 10 x 3 cell footprint of the 3-channel axis-angle grid is preloaded
// into __shared__ once per block (10 x 10 x 3 x 3 x 4 = 3.6 KB), then the
// pixel loop reads through shmem and runs axis_angle_rotate_bwd to derive
// grad_axis_angle exactly as in the original kernel.

#include "BilagridConfig.cuh"

#include "BilagridAxisAngle.cuh"

#ifndef BILAGRID_NORMAL_BWD_V1_SHMEM_CONSTANTS_DEFINED
#define BILAGRID_NORMAL_BWD_V1_SHMEM_CONSTANTS_DEFINED
constexpr int kNormalShmemFootX  = (int)kBilagridBwdV1BlockX + 2;
constexpr int kNormalShmemFootY  = (int)kBilagridBwdV1BlockY + 2;
constexpr int kNormalShmemFootZ  = 3;
constexpr int kNormalShmemChans  = 3;
constexpr int kNormalShmemCells  = kNormalShmemFootX * kNormalShmemFootY * kNormalShmemFootZ;
constexpr int kNormalBlockSize   = (int)kBilagridBwdV1BlockX
                                 * (int)kBilagridBwdV1BlockY
                                 * (int)kBilagridBwdV1BlockZ;
#endif

#ifdef PATCHED
__global__ void bilagrid_normal_patched_sample_backward_v1_kernel_bilagrid_shmem(
#else
__global__ void bilagrid_normal_uniform_sample_backward_v1_kernel_bilagrid_shmem(
#endif
    ...
    const int g_id = ni;
#endif
    const int corner_base = g_id * L * H * W * kNormalShmemChans;
    ...
    __shared__ float shmem_corners
        [kNormalShmemFootZ][kNormalShmemFootY][kNormalShmemFootX][kNormalShmemChans];
    ...
    #pragma unroll
    for (int local = linear_tid; local < kNormalShmemCells; local += kNormalBlockSize) {
        const int lx = local % kNormalShmemFootX;
        const int ly = (local / kNormalShmemFootX) % kNormalShmemFootY;
        const int lz = local / (kNormalShmemFootX * kNormalShmemFootY);
        const int gx = min(max(base_xi + lx, 0), W - 1);
        const int gy = min(max(base_yi + ly, 0), H - 1);
        const int gz = min(max(base_zi + lz, 0), L - 1);
        const int g_off = corner_base + ((gz * H + gy) * W + gx) * kNormalShmemChans;
        #pragma unroll
        for (int ci = 0; ci < kNormalShmemChans; ci++) {
    ...
    float accum[kNormalShmemChans] = {0.0f, 0.0f, 0.0f};

    // 4. Pixel loop -- corners come from shmem.
    if (inside)
#ifdef PATCHED
    ...
    // 5. Writeback (identical to original).
    int out_idx_start = (((ni * L + zi) * H + yi) * W + xi) * kNormalShmemChans;
